    ...
    <v-card rounded="lg">
      ...
      <v-card-text style="max-height: 60vh">
        ...
        <div v-if="step === 1" class="text-center pa-4">
          ...
          <div class="text-body-2 my-3">اختر ملف Excel (.xlsx) يحتوي على العروض.</div>
        ...
        <div v-else-if="step === 2 && sheet">
          <v-alert variant="tonal" class="mb-3" :text="`تم العثور على ${sheet.rows.length} صف. اربط الأعمدة بالحقول.`" />
          <v-select
            v-for="field in IMPORT_FIELDS"
            :key="field.key"
            v-model="mapping[field.key]"
            :items="sheet.headers"
            :label="field.label + (field.default ? ` (افتراضي: ${field.default})` : '')"
            density="compact"
            clearable
            hide-details
            class="mb-2"
        ...
        <div v-else-if="step === 3 && validation">
          <div class="d-flex ga-3 mb-3 flex-wrap">
            <v-chip color="success" variant="tonal">صالح: {{ validation.valid_count }}</v-chip>
            <v-chip color="warning" variant="tonal">مكرر: {{ validation.duplicate_count }}</v-chip>
            <v-chip color="error" variant="tonal">خطأ: {{ validation.error_count }}</v-chip>
          </div>
          <v-alert v-if="validation.errors.length" type="error" variant="tonal" class="mb-2">
            <div class="text-subtitle-2 mb-1">أخطاء:</div>
            <div v-for="e in validation.errors.slice(0, 10)" :key="e.row" class="text-caption">
              صف {{ e.row }}: {{ e.message }}
            </div>
          </v-alert>
          <v-alert v-if="validation.duplicates.length" type="warning" variant="tonal">
            <div class="text-subtitle-2 mb-1">مكررات (ستُتجاهل):</div>
            <div v-for="d in validation.duplicates.slice(0, 10)" :key="d.row" class="text-caption">
              صف {{ d.row }}: {{ d.reason }}
            </div>
          </v-alert>
        </div>
        ...
        <div v-else-if="step === 4 && result" class="text-center pa-4">
          ...
          <div>تم إدراج {{ result.inserted }} عرض، وتم تجاهل {{ result.skipped }}.</div>
        </div>
      </v-card-text>
      <v-card-actions>
        <v-btn v-if="step > 1 && step < 4" variant="text" @click="step -= 1">السابق</v-btn>
        <v-spacer />
        <v-btn v-if="step === 2" color="primary" @click="runValidate">تحقق</v-btn>
        <v-btn
        ...
        <v-btn v-else-if="step === 4" color="primary" @click="close">إغلاق</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
