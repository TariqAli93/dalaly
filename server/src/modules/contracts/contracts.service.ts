...
async function generateCode() {
  const [last] = await db
    ...
    .limit(1);
  return `CTR-${String((last?.code ? Number(last.code.split("-")[1] ?? 0) : 0) + 1).padStart(4, "0")}`;
}
export async function listTemplates() {
  return (
    await db
      ...
      .orderBy(contractTemplates.contractType)
  ).map(api);
}
export async function updateTemplate(
  ...
  const [row] = await db
    ...
    .where(eq(contractTemplates.contractType, contractType))
    .returning();
...
async function bundle(id: number) {
  const [row] = await db
    .select({
      ...
      templateBody: contractTemplates.body,
    })
    .from(contracts)
  ...
  const parties = await db
    .select({
      ...
      customerPhone: customers.phonePrimary,
    })
    .from(contractParties)
    .innerJoin(customers, eq(contractParties.customerId, customers.id))
...
export async function listContracts(filters: ContractFilters) {
  ...
  if (filters.customer_id)
    where.push(eq(contracts.primaryCustomerId, filters.customer_id));
  ...
  const rows = await db
    .select({ contract: contracts, customerName: customers.fullName })
    .from(contracts)
    .leftJoin(customers, eq(contracts.primaryCustomerId, customers.id))
  ...
  const template = payload.template_id
    ...
    : (
        await db
          .select()
          .from(contractTemplates)
          .where(eq(contractTemplates.contractType, payload.contract_type))
          .limit(1)
      )[0];
  return {
    contractType: payload.contract_type,
    primaryCustomerId:
      payload.customer_id ?? payload.parties[0]?.customer_id ?? null,
...
export async function createContract(payload: ContractPayload) {
  ...
  const missingDocuments = payload.parties.length
    ? (
        await Promise.all(
          payload.parties.map((party) =>
            getDocumentRequirements(party.customer_id),
          ),
        )
...
function replaceTemplate(body: string, variables: Record<string, string>) {
  return body.replace(
    /{{\s*([a-z0-9_]+)\s*}}/gi,
...
async function variablesFor(id: number) {
  const [row] = await db
    ...
    .from(contracts)
    .leftJoin(contractTemplates, eq(contracts.templateId, contractTemplates.id))
    .leftJoin(companySettings, eq(companySettings.id, 1))
    .leftJoin(properties, eq(contracts.propertyId, properties.id))
    .leftJoin(rentals, eq(contracts.rentalId, rentals.id))
    .where(eq(contracts.id, id))
    .limit(1);
  if (!row) return null;
  const parties = await db
  ...
  const place = row.property
    ? [
        row.property.governorate,
        row.property.district,
        row.property.neighborhood,
        row.property.addressDetails,
      ]
        .filter(Boolean)
        .join("، ")
    : [
        row.rental?.governorate,
        row.rental?.district,
        row.rental?.neighborhood,
        row.rental?.addressDetails,
      ]
        .filter(Boolean)
        .join("، ");
  return {
    ...
    variables: {
      ...
      company_phone: row.company?.phonePrimary ?? "",
      customer_name: parties[0]?.name ?? "",
      customer_phone: parties[0]?.phone ?? "",
      seller_name: role("seller")?.name ?? "",
      buyer_name: role("buyer")?.name ?? "",
      tenant_name: role("tenant")?.name ?? "",
      landlord_name: role("landlord")?.name ?? "",
      property_address: place,
      contract_amount: row.contract.amount ?? "",
      contract_date: formatDate(row.contract.contractDate),
      start_date: formatDate(row.contract.startDate),
      end_date: formatDate(row.contract.endDate),
      notes: row.contract.notes ?? "",
    },
  };
}
export async function generateContract(id: number) {
  const values = await variablesFor(id);
  if (!values) return null;
  const content = replaceTemplate(values.body, values.variables);
  await db
    .update(contracts)
    .set({
