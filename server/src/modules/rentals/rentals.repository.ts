...
const SEARCHABLE_COLUMNS = [
  ...
  rentals.addressDetails,
  rentals.ownerName,
  rentals.ownerPhone,
  rentals.ownerNotes,
  rentals.otherDetails,
  rentals.notes,
];
...
function searchClause(term: string): SQL {
  const like = `%${term}%`;
  return or(
    ...SEARCHABLE_COLUMNS.map((column) => ilike(column, like)),
    sql`cast(${rentals.rentPrice} as text) ilike ${like}`,
    sql`cast(${rentals.areaValue} as text) ilike ${like}`,
  )!;
}
...
export async function listRentals(filters: RentalFilters) {
  ...
  if (filters.property_type)
    where.push(eq(rentals.propertyType, filters.property_type));
  if (filters.rent_period)
    where.push(eq(rentals.rentPeriod, filters.rent_period));
  if (typeof filters.rent_price_min === "number")
    where.push(gte(rentals.rentPrice, String(filters.rent_price_min)));
  if (typeof filters.rent_price_max === "number")
    where.push(lte(rentals.rentPrice, String(filters.rent_price_max)));
  if (typeof filters.area_min === "number")
    where.push(gte(rentals.areaValue, String(filters.area_min)));
  if (typeof filters.area_max === "number")
    where.push(lte(rentals.areaValue, String(filters.area_max)));
  if (typeof filters.rooms_count === "number")
    where.push(gte(rentals.roomsCount, filters.rooms_count));
  if (typeof filters.bathrooms_count === "number")
    where.push(gte(rentals.bathroomsCount, filters.bathrooms_count));
  if (typeof filters.floors_count === "number")
    where.push(gte(rentals.floorsCount, filters.floors_count));
  if (typeof filters.governorate_id === "number")
    where.push(eq(rentals.governorateId, filters.governorate_id));
  if (typeof filters.district_id === "number")
    where.push(eq(rentals.districtId, filters.district_id));
  if (typeof filters.neighborhood_id === "number")
    where.push(eq(rentals.neighborhoodId, filters.neighborhood_id));
  if (filters.negotiable)
    where.push(eq(rentals.isNegotiable, filters.negotiable === "true"));
  const amenities = parseAmenities(filters.amenities);
  if (amenities)
    where.push(
      sql`${rentals.amenities} @> ${JSON.stringify(amenities)}::jsonb`,
    );
  const search = (filters.q ?? "").trim();
  if (search)
    for (const token of search.split(/\s+/).filter(Boolean))
      where.push(searchClause(token));
  let query = db
    .select()
    .from(rentals)
    .where(and(...where))
    .orderBy(desc(rentals.createdAt), desc(rentals.id))
    .$dynamic();
...
): Promise<Omit<NewRental, "code">> {
  const governorate = payload.governorate_id
    ? await getGovernorateName(payload.governorate_id)
    : (payload.governorate ?? null);
  const district = payload.district_id
    ? await getDistrictName(payload.district_id)
    : (payload.district ?? null);
  const neighborhood = payload.neighborhood_id
...
async function generateRentalCode() {
  const [last] = await db
    .select({ code: rentals.code })
    .from(rentals)
    .where(ilike(rentals.code, "R-%"))
...
export async function updateRental(
  ...
  const [row] = await db
    .update(rentals)
    .set({ ...(await normalizePayload(payload)), updatedAt: new Date() })
    .where(eq(rentals.id, id))
    .returning();
  if (!row) return null;
  const result = toApiObject(row, "rentals");
...
export function getRentalAudit(id: number) {
