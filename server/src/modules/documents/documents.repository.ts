...
export async function listDocuments(filters: DocumentFilters) {
  ...
  if (filters.customer_id)
    where.push(eq(documents.customerId, filters.customer_id));
  if (filters.document_type_id)
    where.push(eq(documents.documentTypeId, filters.document_type_id));
  ...
  return rows
    .map((row) => documentApi(row.document, row.customerName, row.typeName))
    .filter((row) => !filters.status || row.computed_status === filters.status);
}
export async function getDocument(id: number) {
  const [row] = await db
    ...
    .limit(1);
  return row ? documentApi(row.document, row.customerName, row.typeName) : null;
}
...
export async function updateDocument(id: number, payload: DocumentUpdate) {
  const [row] = await db
    .update(documents)
    .set({
      documentTypeId: payload.document_type_id ?? null,
...
export async function getDocumentRequirements(customerId: number) {
  const required = await db
    ...
    .from(documentTypes)
    .where(
      and(eq(documentTypes.isRequired, true), eq(documentTypes.isActive, true)),
    )
    .orderBy(asc(documentTypes.name));
  const uploaded = await db
    .select({ document: documents, typeName: documentTypes.name })
    .from(documents)
    .leftJoin(documentTypes, eq(documents.documentTypeId, documentTypes.id))
    .where(
      and(eq(documents.customerId, customerId), eq(documents.status, "active")),
    );
  const uploadedByType = new Map(
    uploaded
      .filter((row) => row.document.documentTypeId)
      .map((row) => [row.document.documentTypeId, row]),
  );
  const expired = uploaded
    .filter(
      (row) => row.document.expiresAt && row.document.expiresAt < new Date(),
    )
    .map((row) => documentApi(row.document, null, row.typeName));
  const missing = required
    .filter((type) => !uploadedByType.has(type.id))
    .map((type) => toApiObject(type));
  return {
    required: required.map((type) => toApiObject(type)),
    uploaded: uploaded.map((row) =>
      documentApi(row.document, null, row.typeName),
    ),
    missing,
    expired,
  };
