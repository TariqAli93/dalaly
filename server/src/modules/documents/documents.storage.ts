...
export function saveManagedFile(scope: string, data: string, originalName: string | null | undefined, fileType: string) {
  ...
  return { filePath: path.join(relativeDirectory, fileName).replaceAll("\\", "/"), fileSize: buffer.length };
}
export function resolveManagedFile(filePath: string) {
  const root = path.resolve(config.documentsDir);
  const absolute = path.resolve(root, filePath);
  if (absolute !== root && !absolute.startsWith(`${root}${path.sep}`)) throw new Error("مسار الملف غير صالح.");
  return absolute;
}
export function deleteManagedFile(filePath: string) {
  try { const absolute = resolveManagedFile(filePath); if (fs.existsSync(absolute)) fs.unlinkSync(absolute); } catch { /* لا نحذف أي ملف خارج مجلد المستندات. */ }
}
export function contentTypeForFile(filePath: string) {
  const ext = path.extname(filePath).toLowerCase();
  return ({ ".pdf": "application/pdf", ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".png": "image/png", ".webp": "image/webp", ".doc": "application/msword", ".docx": "application/vnd.openxmlformats-officedocument.wordprocessingml.document" } as Record<string, string>)[ext] ?? "application/octet-stream";
}
