...
export const customersRoutes: FastifyPluginAsync = async (app) => {
  app.get(
    ...
    async (request) =>
      listCustomers(customerFiltersSchema.parse(request.query)),
  );
  app.get(
    "/:id",
    { preHandler: requirePermission("customers.read") },
  ...
  app.post(
    ...
    async (request, reply) =>
      reply
        .code(201)
        .send(await createCustomer(customerPayloadSchema.parse(request.body))),
  );
  app.put(
    "/:id",
    ...
    async (request, reply) => {
      ...
      const row = await updateCustomer(
        id,
        customerPayloadSchema.parse(request.body),
      );
      return row ?? reply.code(404).send({ message: "العميل غير موجود." });
    },
  );
