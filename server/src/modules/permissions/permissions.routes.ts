...
export const permissionsRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", { preHandler: requirePermission("roles.read") }, async () =>
    listPermissions(),
  );
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      if (!id)
        return reply.code(400).send({ message: "معرف الصلاحية غير صحيح." });
      const payload = permissionPayloadSchema.parse(request.body);
      const permission = await updatePermission(id, payload);
      if (!permission)
        return reply.code(404).send({ message: "الصلاحية غير موجودة." });
      return permission;
    },
  );
