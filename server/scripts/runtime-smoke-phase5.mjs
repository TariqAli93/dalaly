...
const check = (n, c, e) => {
  if (c) {
    ...
    failed++;
    console.log(
      "  FAIL " + n + (e !== undefined ? " -> " + JSON.stringify(e) : ""),
    );
  }
};
...
async function main() {
  ...
  const logs = [];
  child.stdout.on("data", (c) => logs.push("" + c));
  child.stderr.on("data", (c) => logs.push("" + c));

  try {
    await waitHealth();
    await req("POST", "/setup/initialize", {
      token: null,
      body: {
        host: "127.0.0.1",
        port: 5432,
    ...
    TOKEN = (
      await req("POST", "/auth/login", {
        token: null,
        body: { username: "admin", pin: "4321" },
      })
    ).body.token;
    check("login", !!TOKEN);

    const govId = (
      await req("POST", "/locations/governorates", { body: { name: "بغداد" } })
    ).body.id;
    const distId = (
      await req("POST", "/locations/districts", {
        body: { governorate_id: govId, name: "الدورة" },
      })
    ).body.id;
    const distId2 = (
      await req("POST", "/locations/districts", {
        body: { governorate_id: govId, name: "الرصافة" },
      })
    ).body.id;
    const neighId = (
      await req("POST", "/locations/neighborhoods", {
        body: { district_id: distId, name: "حي دجلة" },
      })
    ).body.id;

    // P: عرض غنيّ بكل الحقول.
    const P = await req("POST", "/properties", {
      body: {
        property_type: "بيت",
        ...
        unit_price: 300000,
        owner_name: "علي حسن الموسوي",
        owner_phone: "0770 123 4567",
        governorate_id: govId,
        district_id: distId,
        neighborhood_id: neighId,
        address_details: "قرب جامع النور شارع 14",
        notes: "بيت زاوية مطل على الشارع",
        plot_number: "321",
        plot_letter: "أ",
        nazal: "12.5",
        frontage: "8",
        subdistrict_number: "م7",
        subdistrict_name: "المقاطعة الشمالية",
        mahalla: "محلة 620",
        alley: "زقاق 5",
        house_number: "دار 12",
        nearest_landmark: "قرب الحديقة",
      },
    });
    ...
    const S = await req("POST", "/properties", {
      body: {
        property_type: "أرض",
        legal_type: "طابو ملك صرف",
        area_value: 500,
    ...
    const Q = await req("POST", "/properties", {
      body: {
        property_type: "مزرعة",
    ...
    check(
      "create R,S,Q (201)",
      R.status === 201 && S.status === 201 && Q.status === 201,
      [R.status, S.status, Q.status],
    );
    ...
    check(
      "اسم المالك يرجع P فقط (لا Q)",
      has(onlyOwner, pId) && !has(onlyOwner, qId),
      count(onlyOwner),
    );

    console.log("== بحث + فلاتر معاً ==");
    const combo = await req(
      "GET",
      `/properties?q=${encodeURIComponent("دجلة")}&governorate_id=${govId}&district_id=${distId}`,
    );
    check(
      "بحث 'دجلة' + محافظة بغداد + منطقة الدورة يرجع P",
      has(combo, pId),
      count(combo),
    );
    const comboNeigh = await req(
      "GET",
      `/properties?q=${encodeURIComponent("الموسوي")}&neighborhood_id=${neighId}`,
    );
    check("بحث 'الموسوي' + حي دجلة يرجع P", has(comboNeigh, pId));
    const comboNone = await req(
      "GET",
      `/properties?q=${encodeURIComponent("الموسوي")}&district_id=${distId2}`,
    );
    check(
      "بحث 'الموسوي' + منطقة الرصافة يرجع 0 (الفلتر يقيّد)",
      count(comboNone) === 0,
      count(comboNone),
    );
    ...
    check(
      "'الموسوي 999' يرجع 0 (لا عقار يجمع الاثنين)",
      count(crossNone) === 0,
      count(crossNone),
    );
    ...
    const page1 = await req(
      "GET",
      `/properties?q=${encodeURIComponent("دجلة")}&limit=2`,
    );
    const page2 = await req(
      "GET",
      `/properties?q=${encodeURIComponent("دجلة")}&limit=2&offset=2`,
    );
    check("limit=2 يرجع 2", count(page1) === 2, count(page1));
    check("limit=2&offset=2 يرجع 1", count(page2) === 1, count(page2));
    const union = new Set(
      [...(page1.body || []), ...(page2.body || [])].map((p) => p.id),
    );
    check("صفحتان تغطّيان 3 عناصر مميّزة", union.size === 3, union.size);

    console.log("== alias search= ==");
    check(
      "alias search= يعمل مثل q=",
      has(
        await req("GET", `/properties?search=${encodeURIComponent("الموسوي")}`),
        pId,
      ),
    );

    console.log("== لا نتائج ==");
    check("نص بلا تطابق يرجع 0", count(await q("xيzغريبq123345679")) === 0);
  } catch (e) {
    failed++;
